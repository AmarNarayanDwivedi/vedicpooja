...
const BlogSlugDetail = () => {
  ...
  const lang = (translations as any)[language] ? language : "en";
  ...
  if (!post) {
    return (
      <div className="text-center p-10">
        ...
        <Link href="/blog" className="text-[#E67E22] hover:underline">
          ← Back to Blog
        </Link>
      </div>
    );
  ...
  const shareUrl = `https://api.whatsapp.com/send?text=${encodeURIComponent(
    `Check out this article: ${(post.title as any)[lang]} - https://www.vedic-pooja.com/blog/${post.slug}`
  )}`;
  const consultUrl = `https://wa.me/8668552465?text=${encodeURIComponent(
    `Namaste 🙏 I want to book a consultation regarding: ${(post.title as any)[lang]}. Please guide me.`
  )}`;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        ...
        <main className="bg-white/80 dark:bg-slate-800/50 backdrop-blur-sm rounded-2xl shadow-lg ring-1 ring-slate-200 dark:ring-slate-700 overflow-hidden">
          <img
            ...
            alt={`${(post.title as any)[lang]} - Vedic astrology and pooja guidance in Pune`}
            loading="lazy"
          ...
          <article className="p-6 md:p-10 font-poppins">
            ...
            <MarkdownRenderer content={(post.content as any)[lang]} />

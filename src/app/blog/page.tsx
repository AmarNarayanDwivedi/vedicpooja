...
import { allPosts, allTags, translations } from "@/data/blogPosts";
...
// --- Helper Components ---
const LanguageSwitcher = ({ currentLang, setLang }: any) => (
  <div className="flex justify-center gap-2 mb-4">
    {Object.keys(translations).map((lang) => (
      <button
        key={lang}
        onClick={() => setLang(lang)}
        className={`px-3 py-1 text-xs font-semibold rounded-full transition-colors duration-200 uppercase ${currentLang === lang
...
const BlogList = ({ posts, onTagSelect, activeTag, lang }: any) => {
      ...
      <div className="flex justify-center flex-wrap gap-2 mb-10">
        {allTags.map((tag: string) => (
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-full transition-colors duration-200 font-poppins ${activeTag === tag
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        ...
        <footer className="mt-12 text-center font-poppins">
          <p className="text-sm text-slate-500 dark:text-slate-400">
            {(translations as any)[language]?.allRightsReserved}
          </p>
        </footer>
      </div>
    </div>

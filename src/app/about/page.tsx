...
const translations: Record<string, any> = {
  EN: {
    ...
    missionText:
      "To bring peace, prosperity, and spiritual well-being to every devotee through the performance of authentic, heartfelt Vedic rituals.",
    visionTitle: "Our Vision",
    visionText:
      "To make the divine knowledge and sacred practices of Vedic pujas accessible to every Hindu household, no matter where they are in the world.",
  ...
  HI: {
    ...
    missionText:
      "प्रामाणिक, हार्दिक वैदिक अनुष्ठानों के प्रदर्शन के माध्यम से प्रत्येक भक्त के लिए शांति, समृद्धि और आध्यात्मिक कल्याण लाना।",
    visionTitle: "हमारा विजन",
    visionText:
      "वैदिक पूजाओं के दिव्य ज्ञान और पवित्र प्रथाओं को दुनिया में कहीं भी हर हिंदू घर के लिए सुलभ बनाना।",
    sanskritQuote: "सर्वे भवन्तु सुखिनः",
...
const CheckBadgeIcon = ({ className }: any) => (
  <svg
    className={className}
...
const Button = ({ children, className = "", href = "#", onClick }: any) => (
  <a
    href={href}
    onClick={onClick}
...
const Badge = ({ children, className = "" }: any) => (
  <span
    className={`inline-flex items-center gap-x-1.5 rounded-full bg-accent/10 px-3 py-1 text-sm font-medium text-accent border border-accent/20 ${className}`}
  >
    {children}
  </span>
);
...
const AnimatedBackground = () => {
  ...
  if (reduceMotion) {
    return (
      <div className="fixed inset-0 w-full h-full z-0 static-star-field"></div>
    );
  }
    ...
    <div className="fixed inset-0 w-full h-full z-0 overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-center justify-center scale-75 md:scale-100">
        ...
        {planets.map((p, i) => (
          <div
            key={i}
            className="absolute rounded-full border border-white/10 orbit-path"
            style={{
              width: `${p.orbit * 2}px`,
              height: `${p.orbit * 2}px`,
      ...
      <div className="absolute bottom-[20%] left-[10%] text-5xl text-accent/20 floating-symbol">
        ॐ
      </div>
      <div
        className="absolute top-[25%] right-[15%] text-5xl text-accent/20 floating-symbol"
      ...
      <div
        className="absolute top-[15%] left-[20%] text-4xl text-primary/15 floating-symbol"
        style={{ animationDelay: "1s" }}
      >
        ☸
      </div>
      <div
        className="absolute bottom-[30%] right-[25%] text-3xl text-accent/15 floating-symbol"
        style={{ animationDelay: "3s" }}
      >
        ☯
      </div>
      <div
        className="absolute top-[40%] left-[5%] text-2xl text-primary/10 floating-symbol"
        style={{ animationDelay: "4s" }}
      >
        🌙
      </div>
      <div
        className="absolute bottom-[10%] right-[10%] text-3xl text-accent/12 floating-symbol"
        style={{ animationDelay: "2.5s" }}
      >
        ⭐
      </div>
      <div
        className="absolute top-[60%] right-[30%] text-2xl text-primary/10 floating-symbol"
        style={{ animationDelay: "1.5s" }}
      >
        🔮
      </div>
      <div
        className="absolute bottom-[50%] left-[15%] text-3xl text-accent/12 floating-symbol"
    ...
    <section className="relative h-[65vh] min-h-[500px] flex items-center justify-center text-center text-secondary overflow-hidden">
      ...
      <div className="container mx-auto px-6 z-10 fade-in-scroll">
        <img
          ...
          style={{
            boxShadow:
              "0 0 20px rgba(230, 126, 34, 0.5), 0 0 40px rgba(230, 126, 34, 0.3), 0 0 60px rgba(230, 126, 34, 0.1)",
          }}
        />
        <h1 className="font-playfair text-4xl md:text-6xl font-bold text-secondary drop-shadow-lg">
          Pandit Aditya Narayan Ji
        </h1>
        <p className="mt-3 font-poppins text-lg md:text-xl text-primary max-w-3xl mx-auto drop-shadow-md">
          {translations[lang].heroTagline}
        </p>
        <div className="mt-8 flex items-center justify-center gap-4">
          <Button onClick={() => router.push("/pooja")}>
            {translations[lang].bookPooja}
          </Button>
        </div>
      </div>
    </section>
...
const StatsSection = ({ lang }: any) => {
  const stats = [
    {
      number: 4000,
      label: translations[lang].statsGuides,
      subtext: translations[lang].statsGuidesSub,
    },
    {
      number: 500,
      label: translations[lang].statsPoojaTypes,
      subtext: translations[lang].statsPoojaTypesSub,
    },
    {
      number: 10000,
      label: translations[lang].statsPoojasDone,
      subtext: translations[lang].statsPoojasDoneSub,
    },
  ];
  return (
...
const IntroFactsSection = ({ lang }: any) => (
  <section className="py-20 md:py-24 relative z-10">
    <div className="container mx-auto px-6 grid md:grid-cols-5 gap-12 items-center">
      ...
      <div
        className="md:col-span-3 text-secondary fade-in-scroll"
        style={{ animationDelay: "0.2s" }}
      >
        <h2 className="font-playfair text-3xl md:text-4xl font-bold text-secondary">
          {translations[lang].lifeDevotedTitle}
        </h2>
        <div className="mt-6 space-y-4 text-secondary/80">
          <p>
            With over 12 years of devoted service, Pandit Aditya Narayan Ji has
            brought peace and spiritual guidance to families across 10+ states.
            Trained under revered Gurus, he follows traditional Vedic methods
            with utmost precision and dedication.
          </p>
          <p>
            He specializes in a wide array of ceremonies, including wedding
            rituals, Graha Pravesh, various Dosh Nivaran Poojas, Havans, and
            providing auspicious Muhurat timings for significant life events.
          </p>
        </div>
        <div className="mt-8 flex flex-wrap gap-3">
          <Badge>
            <CheckBadgeIcon className="w-4 h-4" /> Verified Pandit
          </Badge>
          <Badge>
            <CheckBadgeIcon className="w-4 h-4" /> Traditional Vedic Methods
          </Badge>
          <Badge>
            <CheckBadgeIcon className="w-4 h-4" /> Online Pooja Available
          </Badge>
        </div>
      </div>
    </div>
...
const JourneyTimeline = ({ lang }: any) => {
  const timelineData = [
    {
      year: 2012,
      event: "Started spiritual training under Guru Shastriji",
      highlight: true,
    },
    { year: 2014, event: "First Griha Pravesh Puja", highlight: false },
    {
      year: 2017,
      event: "Expanded to Maharashtra and Gujarat",
      highlight: false,
    },
    ...
    <section className="py-20 md:py-24 bg-gradient-to-br from-primary/5 via-white to-accent/5 relative z-10">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16 fade-in-scroll">
          <h2
            className="font-playfair text-5xl md:text-6xl font-bold text-secondary mb-4"
            style={{ textShadow: "2px 2px 4px rgba(0,0,0,0.1)" }}
          >
            {translations[lang].journeyTitle}
          </h2>
          <p className="text-lg text-secondary/70 max-w-2xl mx-auto">
            A divine journey of spiritual service spanning over a decade
          </p>
        </div>
        <div className="relative max-w-4xl mx-auto">
          ...
          {timelineData.map((item, index) => (
            <div
              key={index}
              className={`flex items-center w-full mb-12 fade-in-scroll ${index % 2 === 0 ? "justify-start" : "justify-end"
                }`}
            >
              <div
              ...
              >
                <Card
                  className={`p-6 milestone-card ${item.highlight
              ...
              <div
                className={`absolute left-1/2 -translate-x-1/2 w-8 h-8 rounded-full shadow-lg z-10 timeline-dot ${item.highlight
...
const MissionVisionSection = ({ lang }: any) => (
  <section className="py-20 md:py-24 relative z-10 overflow-hidden bg-gradient-to-br from-cream via-white to-cream/50">
    ...
    <div className="container mx-auto px-6 text-center max-w-5xl relative z-20">
      <h2
        className="font-playfair text-5xl md:text-6xl font-bold text-secondary fade-in-scroll mb-4"
        style={{ textShadow: "2px 2px 4px rgba(0,0,0,0.1)" }}
      >
      ...
      <div className="mt-12 grid md:grid-cols-2 gap-12 text-secondary/90">
        <div
          className="fade-in-scroll bg-white/80 backdrop-blur-sm rounded-2xl p-8 shadow-xl border border-primary/10 hover:shadow-2xl transition-all duration-300"
          style={{ animationDelay: "0.2s" }}
        >
          <div className="flex items-center mb-4">
            <div className="w-3 h-3 bg-primary rounded-full mr-3"></div>
            <h3 className="text-3xl font-bold text-primary font-playfair">
              {translations[lang].missionTitle}
            </h3>
          </div>
          <p className="text-lg leading-relaxed">
            {translations[lang].missionText}
          </p>
        </div>
        <div
          className="fade-in-scroll bg-white/80 backdrop-blur-sm rounded-2xl p-8 shadow-xl border border-primary/10 hover:shadow-2xl transition-all duration-300"
        ...
        >
          <div className="flex items-center mb-4">
            ...
            <h3 className="text-3xl font-bold text-primary font-playfair">
              {translations[lang].visionTitle}
            </h3>
          </div>
          <p className="text-lg leading-relaxed">
            {(translations as any)[lang].visionText}
          </p>
        </div>
      </div>
      <div className="mt-16 bg-gradient-to-r from-primary/10 to-accent/10 rounded-2xl p-8 border border-primary/20">
        <p
          className="text-3xl font-playfair text-primary/90 italic fade-in-scroll"
          style={{ animationDelay: "0.6s" }}
        >
          {(translations as any)[lang].sanskritQuote}
        </p>
  ...
  return (
    <section className="py-20 md:py-24 bg-primary/5 relative z-10">
      <div className="container mx-auto px-6">
        <div className="text-center mb-12 fade-in-scroll">
          <h2 className="font-playfair text-4xl md:text-5xl font-bold text-secondary">
            {(translations as any)[lang].galleryTitle}
          </h2>
        </div>
        <div className="gallery-grid">
          {images.map((img, i) => (
            <div
              key={i}
              className="gallery-item fade-in-scroll"
      ...
      {modalOpen && (
        ...
        >
          <div
            className="relative bg-cream rounded-lg shadow-xl max-w-3xl w-full max-h-[90vh] overflow-auto text-secondary"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={closeModal}
              className="absolute top-2 right-2 text-secondary bg-white/50 rounded-full p-2 z-10"
            >
              &times;
            </button>
            <img
            ...
            <div className="p-6">
              ...
              <p className="text-secondary/80 mt-2">
                A sacred {selectedImage?.name} ceremony performed with
...
const WhyChooseUsSection = ({ lang }: any) => {
  // ... (This section needs color updates)
  const reasons = [
    {
      title: "Authentic Vedic Practices",
      text: "Rituals performed strictly as per Shastras and Guru Parampara.",
    },
  ...
  return (
    <section className="py-20 md:py-24 relative z-10">
      <div className="container mx-auto px-6">
        <div className="text-center mb-12 fade-in-scroll">
          <h2 className="font-playfair text-4xl md:text-5xl font-bold text-secondary">
            {(translations as any)[lang].whyChooseUsTitle}
          </h2>
        </div>
...
export default function AboutPage() {
  ...
  useEffect(() => {
    ...
    setLang((map as any)[language] || "EN");
    document.title =
      "About Pandit Aditya Narayan Ji | Authentic Vedic Priest & Astrologer";

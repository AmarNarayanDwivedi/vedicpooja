...
const NavLink = ({ to, children }: { to: string; children: React.ReactNode }) => {
  ...
  return (
    <Link
      href={to}
      className={`relative group font-semibold py-2 ${isActive ? "text-[#E67E22]" : "text-gray-700"
        }`}
    >
...
const NavLinkHash = ({ to, children }: { to: string; children: React.ReactNode }) => {
  return (
    <Link
      href={to}
...
export default function Header() {
  ...
  return (
    <header
      className={`header-sticky fixed w-full z-50 top-0 transition-all duration-300 ${isScrolled
    ...
    >
      <div className="container mx-auto px-3 md:px-6 flex justify-between items-center py-4">
        ...
        <nav className="hidden md:flex items-center gap-6">
          ...
          <NavLinkHash to="/#testimonials">{t.navTestimonials}</NavLinkHash>
          <NavLink to="/blog">{t.navBlog}</NavLink>
          <NavLinkHash to="/#contact">{t.navContact}</NavLinkHash>
        </nav>
      ...
      <div
        className={`md:hidden fixed top-0 right-0 h-screen w-80 bg-white shadow-2xl transform transition-transform duration-300 ease-in-out z-40 ${isMenuOpen ? "translate-x-0" : "translate-x-full"
          }`}
      >
